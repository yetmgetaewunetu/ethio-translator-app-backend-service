...
app.post("/summarize-url", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: error.message || "Summarization and translation failed" });
  }
});
...
app.post("/qa", async (req, res) => {
  try {
    ...
    if (!url || !question) {
      return res
        .status(400)
        .json({ error: "Missing required fields: url or question" });
    }
    ...
    res
      .status(500)
      .json({ error: error.message || "Question answering failed" });
  }
});
